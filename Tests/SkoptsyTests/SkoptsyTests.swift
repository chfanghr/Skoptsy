...
//  SkoptsyTests.swift
//
//  Created by 方泓睿 on 2021/8/2.
//

import XCTest
@testable import Skoptsy
...
final class SkoptsyTests: XCTestCase {
        ...
        Module { name in
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test, [val.in1, val.in2], [val.loc.out])
                    }
                    weak {
                        always {
        ...
        Module { name in
            ...
            activity(name.Test, []) { val in
                when { false } abort: {
                    `repeat` {
                        `await` { true }
                        Skoptsy.run(name.Inner, []) { res in val.tmp = res }
                        `return` { val.tmp }
                        `await` { false }
                    }
                }
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test, []) { res in val.out = res }
                    }
                    weak {
                        always {
    ...
    func testCobegin() {
        Module(imports: [common]) { name in
            activity(name.Test, [], [name.done]) { val in
                cobegin {
                    strong {
                        Skoptsy.run(name.Delay, [1])
                    }
                    strong {
                        Skoptsy.run(name.Delay, [3])
                    }
                    strong {
                        Skoptsy.run(name.Delay, [2])
                    }
                    weak {
                        Skoptsy.run(name.Delay, [2])
                    }
                    weak {
                        Skoptsy.run(name.Delay, [4])
                    }
                }
                exec { val.done = true }
            }
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test, [], [val.loc.out])
                    }
                    weak {
                        always {
                            XCTAssertEqual(val.out as Bool, val.expect)
        ...
        Module { name in
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.TestRepeatUntil, [], [val.loc.outRepeatUntil])
                    }
                    weak {
                        Skoptsy.run(name.TestWhileRepeat, [], [val.loc.outWhileRepeat])
                    }
                    weak {
                        always {
        ...
        Module { name in
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test, [val.outer, val.inner], [val.loc.reachedInner, val.loc.seenOuter, val.loc.seenInner])
                    }
                    weak {
                        always {
        ...
        Module { name in
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test, [val.cond], [val.loc.out])
                    }
                    weak {
                        always {
                            XCTAssertEqual(val.out as Bool, val.expect)
        ...
        Module { name in
            ...
            activity(name.Main, []) { val in
                ...
                cobegin {
                    ...
                    weak {
                        Skoptsy.run(name.Test1, [val.test], [val.loc.pos1])
                    }
                    weak {
                        Skoptsy.run(name.Test2, [val.test], [val.loc.pos2])
                    }
                    weak {
                        always {
